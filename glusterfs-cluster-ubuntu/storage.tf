...
data "nebius_compute_image" "centos8" {
...
resource "nebius_compute_instance" "storage_node_a" {
  ...
  boot_disk {
    mode = "READ_WRITE"
    initialize_params {
      image_id = data.nebius_compute_image.centos8.image_id
      type     = "network-ssd"
    }
  }
...
resource "nebius_compute_instance" "storage_node_b" {
  ...
  network_interface {
    subnet_id = nebius_vpc_subnet.net-b.id
  }
  ...
  dynamic "secondary_disk" {
    ...
    content {
      auto_delete = true
      disk_id     = nebius_compute_disk.glusterdisk_b[count.index * var.disk_count_per_vm + secondary_disk.key].id
    }
  }
...
resource "nebius_compute_instance" "storage_node_c" {
  count                     = (var.is_ha) ? var.storage_node_per_zone : 0
  allow_recreate            = true
  allow_stopping_for_update = true
  ...
  network_interface {
    subnet_id = nebius_vpc_subnet.net-c.id
  }
  ...
  dynamic "secondary_disk" {
    ...
    content {
      auto_delete = true
      disk_id     = nebius_compute_disk.glusterdisk_c[count.index * var.disk_count_per_vm + secondary_disk.key].id
    }
  }
